...
- [ ] **Fix `release-please`: not actually generating release PRs since PR #16.** Confirmed via the `release-please.yml` run logs (2026-08-12, after PRs #16/#17/#18 merged) — every recent merge commit fails to parse as a Conventional Commit (`commit could not be parsed: 904be1a Feat/phase2 drizzle schema (#18)`, same for #17/#16), because GitHub's merge is landing the **PR title** as the squash commit on `main`, not the actual Conventional-Commits-formatted commit message that passed `commitlint` pre-merge. Result: release-please sees 0 parseable commits for every path and silently produces no release PR — "No latest release pull request found." Fix is either (a) edit the squash commit message to match Conventional Commits at merge time instead of accepting the default PR-title message, or (b) configure the PR-merge/`release-please` setup so it reads PR titles instead of squash commit bodies. Next task.
