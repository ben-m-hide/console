...
- [ ] `packages/db` has no `.env.example` despite needing `DATABASE_URL` (direct) for `drizzle-kit generate`/`migrate` — noticed while adding `apps/ingestion`'s, out of scope for that change (`packages/db` isn't one of the "both apps" `TODO.md` originally meant)
