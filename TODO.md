...
- [ ] Watch for a shared entity-ingestion-pipeline abstraction in `apps/ingestion` (a generic `ingestEntity<T>()` over the client-fetch/normalize/orchestrate shape `sportmonks-client.ts`/`normalize-competition.ts`/`ingest-competitions.ts` currently repeat just once, for `competitions`) — codebase-design review, 2026-08-14. Same "one adapter is a hypothetical seam, two is a real one" reasoning as the `QueryClient` item above: only one entity pipeline exists today, and fixtures/match_events will need transactions and batch inserts `competitions` doesn't, so it's not yet clear what's actually common vs. incidental. Revisit once `seasons` (the second entity) is built, not before.
