...
- [ ] Scan the existing codebase for violations of `docs/conventions/typescript.md`'s judgment-based rules (inline object types, inline `return { ... }` expressions, unexplained abbreviations, for-loops that are actually pure transforms) and fix them — deliberately deferred from 2026-08-14's convention-system setup, which only fixed what was mechanically Biome-autofixable (`useConsistentArrayType`) plus the naming-convention false positives (third-party property names, raw Sportmonks snake_case fields) needed to keep `bun run lint` green. Judgment-based violations weren't scanned or touched.
