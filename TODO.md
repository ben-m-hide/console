...
- [ ] Confirm the football-analytics domain as the backend's first real feature (schema, ingestion job, API surface) rather than leaving it open per ADR 0008 — ingestion job partially confirmed 2026-08-14 (`competitions` only, see below); `apps/api` still has no real routes (`/health`/`/doc`/`/reference` spike only)
...
- [ ] Add `DATABASE_URL` to `apps/api/.env.example` once `createDb()` is actually wired in — no real route needs DB access yet (still the `/health`/`/doc`/`/reference` spike)
