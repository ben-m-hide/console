...
- [ ] Set up a workflow from GitHub's security/code-scanning template picker (https://github.com/ben-m-hide/console/settings/security_analysis, or the `Actions → New workflow` category filter) — flagged by the user 2026-08-13, not yet actioned since it needs a decision on which one, not just a fetch. **CodeQL is already active** (confirmed via the browser 2026-08-13: "Default setup", last scan recent) so re-adding it isn't the gap. The standout candidate from that category that _is_ still missing: **Dependency Review** (`actions/dependency-review-action`) — runs on `pull_request`, diffs the dependency changes in that PR specifically and fails on a newly-introduced known-vulnerable or license-incompatible package, which is a real gap `bun audit` doesn't cover (that only runs against the current lockfile state on `push`/`pull_request`, not diffed against what the PR actually changed). Same commit-SHA-pinning requirement as every other action in this repo.
