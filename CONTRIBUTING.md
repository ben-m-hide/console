...
**Pinned packages must also be excluded from Dependabot's grouping**, or the pin is pointless — a grouped weekly PR would bundle a pinned package's bump with unrelated ones, which is exactly the "red for a reason unrelated to this PR" failure mode pinning exists to prevent, just one layer up. `.github/dependabot.yml`'s `dev-dependencies`/`production-dependencies` groups both carry an `exclude-patterns` list naming every exactly-pinned package — keep it in sync whenever a new package gets pinned (or unpinned). This was also missed initially and caught by review, same pattern as the GitHub Actions gap above.
