...
3. Write `<apps-or-packages>/<name>/tsconfig.json` extending the root `tsconfig.base.json` (`"extends": "../../tsconfig.base.json"` from `apps/*`/`packages/*`, `"../tsconfig.base.json"` from `infra/`) — don't hand-copy the strictness block (see `docs/adr/` history; the whole reason `tsconfig.base.json` exists is that copy-pasting it 8 times was itself a bug). Only add `tsBuildInfoFile`/`lib`/`types`/`include`/`paths` as needed for this package (`apps/api/tsconfig.json` as a minimal reference). `exactOptionalPropertyTypes` is off in the base (`docs/adr/0017-disable-exactoptionalpropertytypes.md`) — no override needed either way.
