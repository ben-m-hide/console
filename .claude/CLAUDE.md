...
- `-`-prefixed files/folders under `src/routes/` are colocated, excluded from the route tree, still importable (e.g. `players/-index.test.tsx`). Don't export a route's component directly from the route file — breaks `autoCodeSplitting`; instead a route's page component lives in `src/components/pages/<feature>/` (e.g. `PlayersList.tsx`) and the route file just wires it up — split immediately, not gated behind a second consumer. Query options factories live in `src/queries/<entity>/`, not colocated under routes — they're shared across a loader and a component, not route-scoped.
- Promote genuinely shared code to `src/lib`/`src/hooks`/`src/components/common` only once a second consumer actually needs it. A `../../../` import is the signal something's in the wrong place.
...
- `exactOptionalPropertyTypes` is off everywhere (see `docs/adr/0017-disable-exactoptionalpropertytypes.md`) — turned off 2026-08-18 after it kept forcing `T | undefined` widening on optional fields just to forward an already-optional value through another optional field. A required field whose value can be genuinely absent (e.g. `ApiError.status: number | undefined`) still models that with `| undefined` — that's real modeling, not this flag.
