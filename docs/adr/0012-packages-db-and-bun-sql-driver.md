...
- **`UNIQUE(sportmonks_id)` on `seasons` and `ball_positions`** goes beyond what `PROJECT.md` §2's "Constraints required for the upsert strategy" paragraph literally lists (it only names `competitions`/`teams`/`players`/`fixtures` for `sportmonks_id` and `match_events` for `sportmonks_event_id`). Treated as a gap in that paragraph, not a deliberate exclusion — §3's idempotent-upsert rule ("every upsert keyed on the Sportmonks ID, not a blind insert") has no stated reason to exempt either table. Flagged inline in both schema files.
