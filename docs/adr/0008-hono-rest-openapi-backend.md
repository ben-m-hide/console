...
- **No first real feature/domain chosen yet.** `/health` is a plumbing spike, not a feature. The next backend work should pick a concrete domain (m3ter-flavored or arbitrary — still open) rather than continuing to build infrastructure in the abstract.
