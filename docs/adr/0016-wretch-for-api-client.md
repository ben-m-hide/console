...
- `apps/web/src/lib/api/api-error.ts` — `ApiError extends Error` (`message`, `url`, `status` — the reference project's field order and `public readonly` style, adapted with `status`/`url` typed `| undefined` rather than the reference's `status = 0` default, since a genuine network failure has neither. Originally framed as an `exactOptionalPropertyTypes`-driven distinction — **per ADR 0017 (2026-08-18), that flag is off project-wide now; this typing stays anyway, as real modeling independent of it**) and `isApiError()`.
...
- `Client`'s type (`QueryStringAddon & Wretch<QueryStringAddon>` in the reference project) needed its 4th generic param (`ErrorType`) pinned explicitly to `WretchError` here, without which the type `.catcherFallback()` actually produces isn't assignable to a `Client` inferred with the default `ErrorType = undefined`. **Corrected 2026-08-18 (ADR 0017): originally attributed to `exactOptionalPropertyTypes` — re-verified by actually removing the flag project-wide and re-running `tsc`, and the same error persists unchanged.** The real cause is a structural mismatch in `wretch@3.0.9`'s own generic inference for `.catcherFallback()`, unrelated to this project's tsconfig.
...
- **Response validation is unchanged.** `wretch` does zero runtime validation; `safeParse` against `packages/shared`'s Zod schemas remains the trust-boundary check, immediately after `get()` resolves. **Correction 2026-08-19:** "immediately after `get()` resolves" described where `safeParse` ran when this ADR was written — in the `queries/` layer. It has since moved into `apps/web/src/lib/api/resource.ts` (`list()`/`retrieve()`, both built on `get()`), so validation now lives inside `lib/api/`, not outside it. `lib/api/` is no longer a pure-transport-only layer — `api-client.ts`'s `get()` still hands back an unvalidated body, but `resource.ts` sits directly on top of it and validates before returning. This was a deliberate move, not drift: `list()` and `retrieve()` share identical fetch-then-validate logic, and co-locating it beat duplicating a `safeParse` call at every query-options call site. See `docs/design/frontend-architecture.md` §2 for the current file layout.
