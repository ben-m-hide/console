...
5. **Root error boundary and a `notFoundComponent`** — **done**, landed separately from the rewiring as planned. `__root.tsx`'s `Route` now sets `errorComponent` (reuses `GenericError`, title `"console-next"` — catches a render error in the shell itself, which per-route `errorComponent` cannot) and `notFoundComponent` (new `components/common/NotFound.tsx`, an `EmptyState` with a link back to `/`). Covers the CloudFront 403/404→`/index.html`-at-200 case (ADR 0007): a deep link to a nonexistent path now renders an honest message instead of a crash or a blank shell. `NotFoundRoute` (deprecated) was not used.
...
| 4   | Default `staleTime` (5 min)                      | ✅ done  | Landed with #3. Root error boundary + `notFoundComponent` were deliberately split out as error-surface concerns — landed separately, see §3 item 5                                                                                                                                                                                        |
