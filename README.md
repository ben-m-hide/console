...
- **`apps/ingestion`:** first real Sportmonks vertical slice (2026-08-14), `competitions` only — fetch (`Authorization` header, real API), normalize, validate against `packages/shared`'s `CompetitionSchema`, upsert via `packages/db`'s `createDb()` (pooled connection string). Manual trigger only (`bun run src/index.ts`), no scheduling yet. Fixtures/seasons/teams/match_events/ball_positions, rate limiting, and `ingestion_runs` audit logging are deliberately not built yet — see `TODO.md`.
