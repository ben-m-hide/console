...
`apps/web/src/` has `routes/`, `queries/`, `routing/`, `hooks/`, `components/`, `config/`, `lib/`, `test/`. `src/hooks/` was created 2026-08-19 once the first real cross-file hook existed (`hooks/DataTable/useServerDataTable.ts`, shared between `DataTable.tsx` and the players route's URL search-param sync). `src/stores/` is still absent: empty placeholder directories aren't tracked by git (a fresh clone wouldn't have had them, since git doesn't track empty dirs — the same "decorative declaration" class of issue as the removed `.nvmrc`). It gets created for real the moment the first Zustand store actually exists — see "Promote to shared space" below — not before.
...
- `mantine-react-table` (`apps/web`, DataTable) has never published a version supporting Mantine 8/9 — its peer dep caps at `^7.9`, and its built `dist/index.cjs`/`dist/index.esm.mjs` call Mantine's `Collapse` with the old `in` prop, which Mantine 9 renamed to `expanded`; without a fix, the built-in column-filter row, progress bar, and alert banner never render. Patched via `bun patch` (`patchedDependencies` in `package.json`, `patches/mantine-react-table@2.0.0-beta.9.patch`, five `in`→`expanded` renames across both dist files) rather than waiting on upstream. **After changing the patch, clear Vite's dep cache** (`rm -rf apps/web/node_modules/.vite`) — Vite pre-bundles dependencies and will otherwise keep serving the stale pre-patch code even after `bun patch --commit` / a fresh `bun install`.
...
- `run-web` (`apps/web/.claude/skills/`, named `run-console` until it was renamed for consistency with the workspace directory) and `run-api` (`apps/api/.claude/skills/`), each colocated with the unit it drives — produced by `/run-skill-generator`, one per deployable unit. Each launches its app for real and drives it (`run-web` now delegates to the real `packages/e2e` Playwright Test suite rather than the bespoke driver script it started with; `run-api` uses a `curl`-based smoke script), documenting only commands actually run and verified, not paraphrased from other docs. `run-api` surfaced a real gotcha worth knowing generally: port 3000 can have two listeners at once (broad `*:3000` vs. loopback-only `[::1]:3000`) with no bind error, silently routing requests to the wrong process — see that skill's Gotchas section.
