...
**Two response shapes, deliberately:** paginated routes return an envelope `{ data, meta: { page, pageSize, total, totalPages } }`, because a client cannot render pagination without knowing `total`. Unpaginated routes (`/competitions`, `/seasons`) return a bare array. Use the envelope only where pagination genuinely requires it — it is not the default house style.
