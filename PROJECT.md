...
| `GET /api/v1/seasons?competition=`                      | Season list for the scope selector — flat, not nested under a competition, since the selector may need seasons before one is chosen                                                                                                                         |
| `GET /api/v1/players?search=&position=&page=&pageSize=` | Player search/filter (paginated — see below). **No `team` filter**: `players` has no team column, so team association exists only through `player_season_stats` and is inherently season-scoped. Add it with a required season param when a screen needs it |
...
**Pagination — implemented 2026-08-16 on `/players`:** 1-based `page` plus `pageSize` (default 25, hard cap 50), offset-based rather than cursor. Offset was chosen because the row count is small and only changes when ingestion runs, and because the frontend wants a bookmarkable `?page=` with jump-to-page, which cursors do not give. Results are ordered `name` then `id` — the `id` tiebreak is **required for correctness**, not cosmetic: without a total order, rows shift between pages and the client sees duplicates and gaps.

**One response shape for every list endpoint — `{ data }`, `meta` added only where pagination needs it.** Originally two shapes, deliberately: paginated routes got the envelope (`{ data, meta: { page, pageSize, total, totalPages } }`, since a client can't render pagination without `total`) and unpaginated routes (`/competitions`, `/seasons`) returned a bare array. **Corrected 2026-08-18**: unified to one envelope shape after `apps/web`'s generic `list()`/`listQueryOptions()` (`docs/adr/0017-disable-exactoptionalpropertytypes.md`'s sweep surfaced this) had to special-case the bare-array response — consistency across every list endpoint won over the "don't add structure you don't need yet" argument for the two unpaginated ones. `/competitions` and `/seasons` now return `{ data }` with no `meta` key at all (not an empty/fake one) — `meta` is present only when there's real pagination info to put in it.
...
- [x] Build Sportmonks API client (typed, using `packages/shared` schemas to validate responses) — done 2026-08-14 for `competitions`/`League` and `seasons`/`Season` (`apps/ingestion/src/sportmonks-client.ts`). **Base URL corrected against the real API, not the docs as originally written:** `https://api.sportmonks.com/v3/football/...` — no `/api/` segment, unlike this item's original text. Authenticates via the `Authorization` header, not `?api_token=`, as planned. Shared fetch/error-handling extracted into `fetchSportmonks<T>()` once `fetchSeasons` gave it a second real consumer.
...
- [x] Manually trigger a run against one real competition/season, verify data lands correctly — done 2026-08-14: `bun run src/index.ts` against the real Sportmonks trial API and Neon's `local-dev` branch. 5 competitions + 15 seasons fetched and upserted, 0 failures, verified via a direct `information_schema`-independent `SELECT` joining seasons to competitions (not just trusting the app's own log line). Also found `competitions.tier` had no real Sportmonks data source — dropped from the schema, see the correction note below.
...
- [ ] Implement `/players`, `/players/:id/stats` (with pagination on `/players`) — **`GET /api/v1/players` done 2026-08-16** (paginated, `search`/`position` filters; no `team` filter, see §4) along with a new `GET /api/v1/seasons` that §4 had never listed. `/players/:id/stats` not started.
