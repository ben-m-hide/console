...
| `match_events`        | Every pass/shot/touch/defensive action, event-level | `id`, `fixture_id` (FK), `sportmonks_event_id`, `type`, `player_id` (FK), `related_player_id` (FK, nullable — e.g. pass receiver), `minute`, `x`, `y`, `end_x` (nullable), `end_y` (nullable), `outcome`, `xg` (nullable), `body_part`, `situation` |
...
- `<Pitch>` — renders the pitch outline, takes children as an overlay layer. **Get the viewBox aspect ratio right before drawing anything on it** — a real pitch is roughly 105m × 68m, not square. Check Sportmonks' documented coordinate convention (likely normalized 0–100 on both axes regardless of true pitch shape) and set the SVG `viewBox` to match the real ratio (e.g. `0 0 100 64`), not a naive `0 0 100 100` — otherwise every visualization built on top renders a stretched or squashed pitch.
- `<ShotMap events={shots} />` — circles sized by `xg`, colored by outcome
- `<PassNetwork averagePositions={...} passEdges={...} />` — renders the pre-computed average positions and pass volumes returned by `/fixtures/:id/report` (§4); the aggregation happens server-side, not in this component — keep it a pure renderer
