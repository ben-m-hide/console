...
| `GET /api/v1/fixtures/:id/report`               | **Match Report data**, pre-shaped: `{ shots: [...], passes: [...], xgFlow: [...], averagePositions: [...] }` — one call powers the whole screen |
...
**Correction, Phase 3 (2026-08-12):** `shots` and `passes` as coordinate-bearing payloads aren't buildable — see §2/§11 Phase 3. `xgFlow` (aggregate, team/player-scoped xG over time) and lineup-derived `averagePositions` are unaffected. What replaces `shots`/`passes` in this response, if anything, is the open call flagged in §5.
...
- **Open call (Phase 3, 2026-08-12):** `ballcoordinates` (team/ball-level tracking, not per-player — see §11 Phase 3) is confirmed available on this subscription and could replace `<ShotMap>`/`<PassNetwork>` with a team-level ball position heat map or trajectory on `<Pitch>`. Decide: build against this, or drop the pitch-visualization feature from scope entirely.
...
- [ ] Define Zod schemas in `packages/shared` for: competition, season, team, player, fixture, match event, player season stats
...
- [ ] Add indexes: `match_events(fixture_id)`, `match_events(fixture_id, type)`, `player_season_stats(player_id, season_id)`
...
- [ ] Build `<ShotMap>`, `<PassNetwork>` overlay components (rendering pre-computed data from `/report`, not computing aggregates client-side)
